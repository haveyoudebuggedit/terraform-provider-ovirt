...
When writing the schema you should make sure to provide ample description and validation so that users can reasonably write their Terraform code without tripping over low level errors. The [validation.go](internal/ovirt/validation.go) file already contains a number of validators you can add to your schema.
...
Next, you will need to add the resource in [provider.go](internal/ovirt/provider.go);
